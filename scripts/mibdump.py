...
"""
    ...
    if opt[0] == "--python-optimization-level":
        try:
            ...
            sys.stderr.write(
                "ERROR: known Python optimization levels: -1, 0, 1, 2\r\n%s\r\n"
                % helpMessage
            )
            sys.exit(EX_USAGE)
...
if inputMibs:
    mibSources = (
        sorted(
            {os.path.abspath(os.path.dirname(x)) for x in inputMibs if os.path.sep in x}
        )
        + mibSources
    )
...
try:
    ...
    if verboseFlag:
        ...
        sys.stderr.write(
            "Up to date MIBs: %s\r\n"
            % ", ".join(
                ["%s" % x for x in sorted(processed) if processed[x] == "untouched"]
            )
        )

        sys.stderr.write(
            "Missing source MIBs: %s\r\n"
            % ", ".join(
        ...
        sys.stderr.write(
            "Ignored MIBs: %s\r\n"
            % ", ".join(
                ["%s" % x for x in sorted(processed) if processed[x] == "unprocessed"]
            )
        )
