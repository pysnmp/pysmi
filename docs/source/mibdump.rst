...
pysmi ships its own copies of the base MIBs (SNMPv2-SMI and similar; see
:ref:`bundled-mibs` for the inventory and its counts) and searches them
alongside --mib-source, so more than one source can
...
That case is not a corner: **a good many bundled modules carry no
