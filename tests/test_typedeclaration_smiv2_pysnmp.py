...
class TypeDeclarationTestCase(unittest.TestCase):
    ...
    def TestTextualConventionDisplayHint(self):
        self.assertEqual(self.ctx["TestTextualConvention"].getDisplayHint(), "1x:", "bad DISPLAY-HINT")

    def TestTextualConventionStatus(self):
        self.assertEqual(self.ctx["TestTextualConvention"].getStatus(), "current", "bad STATUS")

    def TestTextualConventionDescription(self):
        self.assertEqual(self.ctx["TestTextualConvention"].getDescription(), "Test TC", "bad DESCRIPTION")

    def TestTextualConventionReference(self):
        self.assertEqual(self.ctx["TestTextualConvention"].getReference(), "Test reference", "bad REFERENCE")

    def TestTextualConventionClass(self):
        self.assertEqual(self.ctx["TestTextualConvention"].__class__.__name__, "TextualConvention", "bad SYNTAX class")
