...
class ModuleIdentityTestCase(unittest.TestCase):
    ...
    def testModuleIdentityRevisions(self):
        self.assertEqual(self.ctx["testModule"].getRevisions(), ("2000-01-10 00:00",), "bad REVISIONS")
    ...
    def testModuleIdentityContactInfo(self):
        self.assertEqual(
            self.ctx["testModule"].getContactInfo(), "WG-email: agentx@dorothy.bmc.com", "bad CONTACT-INFO"
        )

    def testModuleIdentityDescription(self):
        self.assertEqual(
            self.ctx["testModule"].getDescription(), "This is the MIB module for the SNMP", "bad DESCRIPTION"
        )

    def testModuleIdentityClass(self):
        self.assertEqual(self.ctx["testModule"].__class__.__name__, "ModuleIdentity", "bad SYNTAX class")
