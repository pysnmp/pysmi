...
class NotificationGroupTestCase(unittest.TestCase):
    ...
    def testNotificationGroupDescription(self):
        self.assertEqual(
            self.ctx["testNotificationGroup"].getDescription(), "A collection of test notifications.", "bad DESCRIPTION"
        )

    def testNotificationGroupClass(self):
        self.assertEqual(self.ctx["testNotificationGroup"].__class__.__name__, "NotificationGroup", "bad SYNTAX class")
