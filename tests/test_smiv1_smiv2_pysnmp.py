...
class SmiV1TestCase(unittest.TestCase):
    ...
    def testSmiV1Symbol(self):
        self.assertTrue("testSmiV1" in self.ctx, "symbol not present")

    def testSmiV1Name(self):
        self.assertEqual(self.ctx["testSmiV1"].getName(), (1, 3), "bad name")

    def testSmiV1Description(self):
        self.assertEqual(
            self.ctx["testSmiV1"].getDescription(), "A collection of test notifications.", "bad DESCRIPTION"
        )

    def testSmiV1Class(self):
        self.assertEqual(self.ctx["testSmiV1"].__class__.__name__, "NotificationGroup", "bad SYNTAX class")
