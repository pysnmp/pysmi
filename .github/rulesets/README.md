...
green. `test-consumer (pysnmp, non-gating)` is left out of it on purpose: a
red there means pysnmp moved, not that pysmi is broken.
...
- **Tag protection.** Worth having on `refs/tags/v*`, and nothing in this
  repository would break under it. It is left out only because this change
  covers `main` and `next`; the sibling repositories move semver alias tags on
  purpose after a release, so a tag policy cannot be shared verbatim across the
  four.
